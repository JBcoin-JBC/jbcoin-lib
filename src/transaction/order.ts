...
function createOrderTransaction(
  ...
  const takerPays = utils.common.toJbcoindAmount(order.direction === 'buy' ?
    order.quantity : order.totalPrice)
  const takerGets = utils.common.toJbcoindAmount(order.direction === 'buy' ?
  ...
  if (order.expirationTime !== undefined) {
    txJSON.Expiration = iso8601ToJbcoinTime(order.expirationTime)
  }
