...
import {validate, iso8601ToJbcoinTime, jbcToDrops} from '../common'
...
function createEscrowCreationTransaction(account: string,
  ...
  const txJSON: any = {
    ...
    Amount: jbcToDrops(payment.amount)
  }
  ...
  if (payment.allowCancelAfter !== undefined) {
    txJSON.CancelAfter = iso8601ToJbcoinTime(payment.allowCancelAfter)
  }
  if (payment.allowExecuteAfter !== undefined) {
    txJSON.FinishAfter = iso8601ToJbcoinTime(payment.allowExecuteAfter)
  }
