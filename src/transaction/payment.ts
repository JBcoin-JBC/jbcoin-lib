...
export interface Payment {
  ...
  // would like the Jbcoin Network to disregard any direct paths from
...
function isJBCToJBCPayment(payment: Payment): boolean {
...
function createMaximalAmount(amount: Amount): Amount {
  ...
  if (amount.currency === 'JBC') {
    ...
    maxValue = jbcToDrops(maxJBCValue)
  } else {
...
function createPaymentTransaction(address: string, paymentArgument: Payment
  ...
  const amount =
    (isMinAdjustment(payment.destination) && !isJBCToJBCPayment(payment))
