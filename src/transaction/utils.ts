...
function formatPrepareResponse(txJSON: any): Prepare {
  const instructions = {
    fee: common.dropsToJbc(txJSON.Fee),
...
function prepareTransaction(txJSON: any, api: JbcoinAPI,
  ...
  function prepareFee(): Promise<object> {
    ...
    if (instructions.fee !== undefined) {
      ...
      if (fee.greaterThan(api._maxFeeJBC)) {
        const errorMessage = `Fee of ${fee.toString(10)} JBC exceeds ` +
          `max of ${api._maxFeeJBC} JBC. To use this fee, increase ` +
          '`maxFeeJBC` in the JbcoinAPI constructor.'
        throw new ValidationError(errorMessage)
      }
      txJSON.Fee = scaleValue(common.jbcToDrops(instructions.fee), multiplier)
      return Promise.resolve(txJSON)
    }
    ...
    return api.getFee(cushion).then(fee => {
      return api.connection.getFeeRef().then(feeRef => {
        ...
        const feeDrops = common.jbcToDrops(fee)
        const maxFeeJBC = instructions.maxFee ?
          BigNumber.min(api._maxFeeJBC, instructions.maxFee) : api._maxFeeJBC
        const maxFeeDrops = common.jbcToDrops(maxFeeJBC)
