...
import {JbcoindError, DisconnectedError, NotConnectedError,
  TimeoutError, ResponseFormatError, ConnectionError,
  JbcoindNotInitializedError} from './errors'
  ...
  _onOpen() {
    ...
    return this.request(request).then((data: any) => {
      if (_.isEmpty(data) || !data.ledger_index) {
        // jbcoind instance doesn't have validated ledgers
        return this._disconnect(false).then(() => {
          throw new JbcoindNotInitializedError('Jbcoind not initialized')
        })
      }
    ...
    return new Promise((resolve, reject) => {
      ...
      this.once(eventName, response => {
        if (response.status === 'error') {
          _reject(new JbcoindError(response.error, response))
