...
function adjustQualityForJBC(quality, takerGetsCurrency, takerPaysCurrency) {
    // quality = takerPays.value/takerGets.value
    // using drops (1e-6 JBC) for JBC values
    const numeratorShift = (takerPaysCurrency === 'JBC' ? -6 : 0);
    const denominatorShift = (takerGetsCurrency === 'JBC' ? -6 : 0);
    const shift = numeratorShift - denominatorShift;
    return shift === 0 ? quality :
        (new bignumber_js_1.default(quality)).shift(shift).toString();
}
exports.adjustQualityForJBC = adjustQualityForJBC;
...
function parseTimestamp(jbcoinTime) {
    if (typeof jbcoinTime !== 'number') {
        return undefined;
    }
    return common.jbcoinTimeToISO8601(jbcoinTime);
}
