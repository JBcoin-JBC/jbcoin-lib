...
var uglifyOptions = {
  mangle: {
    reserved: ['_', 'JbcoinError', 'JbcoindError', 'UnexpectedError',
...
function getWebpackConfig(extension, overrides) {
  ...
  let defaults = {
    ...
    output: {
      ...
      filename: ['jbcoin-', extension].join(pkg.version)
    },
    plugins: [
...
function createBuildLink(callback) {
  return function(err, res) {
    createLink('./build/jbcoin-' + pkg.version + '.js',
      './build/jbcoin-latest.js');
    callback(err, res);
  };
}
...
gulp.task('build-min', function(callback) {
  ...
  webpack(webpackConfig, function() {
    createLink('./build/jbcoin-' + pkg.version + '-min.js',
      './build/jbcoin-latest-min.js');
    callback();
  });
});
